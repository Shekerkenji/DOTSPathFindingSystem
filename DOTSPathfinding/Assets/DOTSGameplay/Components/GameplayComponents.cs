...
namespace Shek.ECSGameplay
{
    ...
    /// <summary>
    /// Enabled when the player has manually issued a move command to this unit.
    /// While enabled:
    ///   - ThreatScanSystem skips the unit (no target acquisition).
    ///   - AIDecisionSystem skips the unit (no autonomous movement or attacks).
    ///   - CurrentTarget is cleared so combat does not resume mid-move.
    /// Disabled automatically by PlayerControlledSystem when the unit stops moving
    /// (StoppedMoving event fires), handing control back to the AI.
    /// Baked disabled by CombatAgentAuthoring.
    /// </summary>
